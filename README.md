...
| `crates/xusdc-genesis/` | Rust crate: the genesis tool — derives the faucet and role accounts fully offline (deterministic ids before any network exists) and emits the node's genesis inputs: the `.mac` account files, a `genesis.toml` fragment, and an id summary. |
