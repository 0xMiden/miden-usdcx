...
| `crates/xusdc-genesis/` | Rust crate: the genesis tool — builds the genesis xUSDC faucet fully offline (its id deterministic before any network exists) from the configured role-account ids, and writes the faucet's `.mac` account file plus a stdout id listing. |
