...
**Gate:** MockChain (NOT local-node). **Pins:** protocol v0.15.3 (`681fc9058`) → assembler 0.23.3, seeded `Cargo.lock` (isolated workspace). **Status:** PASS — `cargo test --locked --no-fail-fast` green (3 tests). **GO** for the burn slices on these primitives.
...
| Result | `test result: ok. 3 passed; 0 failed` (`c1_next_block_user_create_retrieve_consume`, `c2_same_block_erasure_unauthenticated_consume`, `b_zero_asset_note_traps_wrong_number_of_assets`) |
...
- **Retrieval at block N:** `is_note_committed(id)` true; `get_public_note(id)` returns the full public note; `is_note_unspent(nullifier)` true.
...
**B — exactly-one-asset trap (`b_zero_asset_note_traps_wrong_number_of_assets`).**
- A 0-asset note bearing the stock burn script, consumed by the faucet, traps with the stock `ERR_FUNGIBLE_BURN_WRONG_NUMBER_OF_ASSETS` — asserted by the named error via `assert_transaction_executor_error!`, not `is_err()`.
...
```
`cargo` runs with the sandbox disabled (it writes its package cache under `~/.cargo`, the crate `target/`, and reads the SSH signing key), as the precedent canaries document. Expect `test result: ok. 3 passed; 0 failed`.
