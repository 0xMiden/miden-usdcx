...
//! ([`withdrawal_api`](crate::withdrawal_api)) build on: the base-URL join, the auth-header
//! injection point, the transport seam, and the response-size ceiling.
//!
...
//! [`RetryPolicy`] and the documented [`RateGovernor`] ceilings — because they are properties of
//! the peer being talked to, not of one call: the [`retry`](crate::circle::retry) module's backoff
//! loop reads both off the client, so a driver cannot forget to rate-limit or retry unboundedly by
//! construction.
//!
...
//!
//! 1. **A configured key requires HTTPS** ([`ListenerError::InsecureAuthTransport`]) — over
//!    plaintext the key is readable by anything on the path.
//! 2. **No redirect is ever followed** ([`ReqwestTransport`] builds with
//!    `redirect::Policy::none()`), so there is no path on which the key is re-sent to an origin the
//!    peer chose (no auth scheme is documented, so the key may ride under any header name).
...
/// The default response-size ceiling: 1 MiB. A Circle withdrawal object (and even a five-batch
/// array of them) is kilobytes; a body past this is refused rather than buffered whole.
...
/// How a status poll paces itself: the wait between non-terminal polls, and the attempt ceiling
/// that bounds [`poll_status`](crate::withdrawal_api::poll_status) so it cannot loop forever on a
/// never-terminal status.
///
/// This is NOT the [`RetryPolicy`] (which governs transient HTTP FAILURES) — it is only the cadence
/// of a poll-to-terminal loop over a healthy endpoint, where every answer is a successful `200`
/// that simply is not terminal yet.
...
pub struct CircleClient {
    ...
    /// therefore rebuild when the ceiling changes). A caller-supplied transport owns its own
    /// limits.
    transport_is_default: bool,
    /// The SINGLE source of the response-size ceiling. The default [`ReqwestTransport`] is
    /// (re)built with exactly this value, so the production streaming enforcement and the
    /// client-side post-check never diverge.
    max_response_bytes: usize,
    poll: PollPolicy,
    /// The bounded-retry/backoff policy the submit path runs its attempts under.
    retry: RetryPolicy,
    /// The documented rate ceilings. An `Arc` because the GLOBAL ceiling is only global if every
    /// client in the process shares ONE governor — a per-client governor would silently turn 35 QPS
    /// global into 35 QPS *each*.
...
impl CircleClient {
    ...
    /// * [`ListenerError::InsecureAuthTransport`] — a credential is configured and `base_url` is
    ///   not HTTPS.
    ...
    /// The client the operator's config describes: base URL, and the auth posture the config asks
    /// for (a key iff one is configured — never a hardcoded one).
    ///
    /// # Errors
    /// As [`Self::new`].
    ...
    /// socket); the contract tests install the in-process mock Circle server, so the driver's real
    /// request-building, status policy and decoders run against a real router with no network
    /// involved.
    #[must_use]
    ...
    /// REBUILT with the new ceiling, so the production streaming enforcement uses exactly this
    /// value — a lower ceiling is not left checked only after a default-sized buffer, and a higher
    /// one is not still rejected early at the default. A caller-supplied transport owns its own
    /// limits and is left untouched, but the client-side post-check still applies this ceiling to
    /// it.
    #[must_use]
    ...
    /// Sets the bounded-retry/backoff policy the submit path applies to TRANSIENT failures.
    #[must_use]
    ...
    /// ceiling is a per-process budget, and one governor per client would multiply it by the number
    /// of clients.
    #[must_use]
    ...
    /// The base URL's host — the key the per-IP ceiling is enforced under. The constructor refuses
    /// a base URL with no host, so this is always present.
    ...
    /// The response-size ceiling — the SINGLE source shared with the default production transport
    /// (it is (re)built with exactly this value) and applied by the client-side post-check.
    pub fn max_response_bytes(&self) -> usize {
        self.max_response_bytes
    }

    /// The ceiling the **installed** transport is configured with, read off the transport ITSELF
    /// (not a mirrored client field) when it is a [`ReqwestTransport`]; `None` when the installed
    /// transport is some other type.
    ///
    /// Because it downcasts the actual `Arc<dyn HttpTransport>`, it returns the DEFAULT ceiling if
    /// [`Self::with_max_response_bytes`] fails to rebuild the default transport — so a test
    /// asserting the configured value catches a removed rebuild, and the propagation is observable
    /// without a socket.
    ...
    /// Builds a `GET` request for `path`, joined onto the base URL, with the auth header injected
    /// iff one is configured.
    ...
    /// Builds a `POST` request for `path` with `body` serialized as JSON, the auth header injected
    /// iff one is configured. Uses `reqwest`'s own JSON body codec, so the mock receives exactly
    /// the bytes the production client would put on the wire.
    ...
    /// Executes a built request through the installed transport, **under the rate ceilings**,
    /// enforcing the response-size ceiling.
    ///
    ...
    /// point: this is the one choke point every Circle request funnels through — the `prepare`
    /// POST, the `withdraw` POST and each of its retries, and every `GET /v1/withdrawal/{id}` a
    /// `409` recovery polls. A governor wrapped around the retry loop instead would cover the POST
    /// attempts and silently miss the rest, which is not a smaller ceiling but no ceiling at all on
    /// the paths it misses — and a recovery poll loop is precisely where the request count explodes
    /// (a GET per poll, per conflict, with concurrent conflicts running independent loops).
    ///
    /// Because it sits at the boundary, "a new driver forgets to rate-limit" is not a mistake that
    /// can be made: a driver that does not come through here cannot reach the transport.
    ///
    /// It is also taken exactly ONCE per request. Acquiring at two layers would not be conservative
    /// — it would spend two permits per request and silently run the service at half the documented
    /// rate.
    ///
    /// # Errors
    ...
    ///   production transport already bounds its read, but the ceiling is the CLIENT's policy, so
    ///   it holds for every transport (the mock included).
    pub(crate) async fn execute(
        &self,
        request: reqwest::Request,
    ) -> Result<RawResponse, ListenerError> {
        // waits until this request fits inside both documented ceilings
