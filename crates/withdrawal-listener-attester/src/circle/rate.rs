//! The rate governor — **5 QPS/IP, 35 QPS global** (§10.12, `CIRCLE-API-SURFACE.md:18`, tagged "at
//! NDA writing").
//!
//! A permit is taken before **every request** — the `prepare` POST, the `withdraw` POST and each of its
//! retries, and every `GET /v1/withdrawal/{id}` a `409` recovery polls. It is taken in
//! [`CircleClient::execute`](super::client::CircleClient), the one boundary they all pass through, so
//! the coverage is structural: a driver cannot make a Circle request that the ceiling does not see.
//!
...
//!
//! Sharing one governor (an `Arc`) across clients is what makes the GLOBAL ceiling genuinely global.
//!
...
//! not a wire format: the single-owner rule binds the formats and codecs the two services must agree
//! on (they live in `xusdc-encoding` and are consumed by reference), and a cross-dependency between
//! two sibling services just to share a window would couple them for no protection.
...
/// The documented per-IP ceiling (§10.12).
pub const DOCUMENTED_QPS_PER_IP: u32 = 5;
/// The documented global ceiling (§10.12).
...
/// 0.99s + 5 at 1.01s = 10 requests inside 20ms), which is precisely the burst a ceiling exists to
/// prevent.
///
...
pub struct RateGovernor {
    ...
    ///
    /// It is a genuine operational counter (an ops dashboard wants exactly this), and it is also the
    /// seam that makes the ceiling's COVERAGE checkable rather than merely timed: "did this request
    /// take a permit at all?" is a question a stopwatch answers only when the ceiling happens to bind,
    /// and an ungoverned request is invisible to a stopwatch that is not looking at the right second.
    granted: AtomicUsize,
}
...
impl RateGovernor {
    ...
    /// attempt-ceiling clamp; 0 QPS is never a legitimate configuration of a service whose job is to
    /// make requests.)
    ...
    /// The ceilings as DOCUMENTED (§10.12: 5 QPS/IP, 35 QPS global, "at NDA writing"). Mirrored from
    /// the API surface, never invented — and tagged as Circle's to change.
    ...
    /// How many permits this governor has granted, ever — i.e. how many requests it actually admitted.
    ///
    /// Monotonic, and NOT the window contents (those are pruned every second). It answers the coverage
    /// question the ceilings depend on: a request that never took a permit is a request the ceiling
    /// never saw, and it is invisible to any timing measurement taken while the ceiling is not binding.
    ...
    /// find another took the slot it was waiting for simply waits again, so the invariant ("never
    /// more than N inside any 1s window") holds no matter how many callers race.
