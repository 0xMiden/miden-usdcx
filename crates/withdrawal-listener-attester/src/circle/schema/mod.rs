//! The Circle withdrawal wire schema — serde types matching the OpenAPI EXACTLY (the documented
//! policy).
//!
//! Split by direction of travel, one family per module:
//!
...
//!   bare batch. Flattening one is the classic simplification, and it produces a body Circle
//!   rejects.
...
//!   `metadata.sender`); Circle fills `sourceDepositor` server-side. Swapping them would
...
//! inbound or outbound. (Circle documents "malformed Circle response → reject, not signed.")
//!
...
//!   deserialize through `super::wire::present_non_null`: omitting `salt` means "no salt";
//!   sending `"salt": null` is refused rather than silently read as absence.
...
//!   real money.
//!
//! What is NOT constrained is what the OpenAPI does not document: `encoded`, `messageHashToSign`,
//! and the request-side `burnTxId` are typed `string` with no pattern, so they stay `String`.
