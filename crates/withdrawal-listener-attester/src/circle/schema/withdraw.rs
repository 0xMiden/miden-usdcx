//! `POST /v1/withdraw` and `GET /v1/withdrawal/{withdrawalId}` — the
...
/// `WithdrawBatch.burnSignatures`: "multiple signatures are needed to meet the multi-sig
/// threshold".
...
/// contract — signatures in ascending signer-address order, no duplicates — is NOT expressible in
/// a JSON schema and is the quorum assembler's job.
///
/// So the signature list is carried **verbatim**: never sorted, never deduplicated. A wire type
/// that tidied it would repair the batch on its way out and hide from the quorum check the very
/// defect the quorum check exists to catch.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(
...
pub struct WithdrawBatch {
    ...
    /// The Miden burn transaction identifier — the burn-evidence field Circle resolves against a
    /// Miden node (the burn-evidence format is still OPEN, pending Circle confirmation). The
    /// REQUEST-side field has **no documented pattern** (unlike the response side's
    /// `^0x[a-fA-F0-9]+$`), so none is imposed.
    burn_tx_id: String,
...
impl WithdrawBatch {
    ...
    /// * [`SchemaError::SignatureCountBelowThreshold`] — fewer than 2 signatures. Circle will not
    ///   meet its multi-sig threshold with one, so such a batch must never be submitted.
    pub fn new(
        burn_intents: Vec<BurnIntent>,
        burn_signatures: Vec<HexBytes>,
        burn_tx_id: String,
...
///
/// A `409` is a **duplicate conflict requiring recovery/reconciliation — NOT a success**, and never
/// a blind re-send. This body is the only thing that says which withdrawal the duplicate collided
/// with, so decoding it is the difference between recovering the real state and guessing.
///
/// * `burnTxId` is **required**. It is the idempotency key, and the ONLY thing that binds this
///   conflict to the burn that was submitted — Circle's documented "a 409 body that does not echo
///   the original `burnTxId` → treat as a defect" is uncheckable without it. A `409` whose body
///   carries no `burnTxId` therefore does not decode, becomes an exact
///   [`ListenerError::MalformedResponse`](crate::error::ListenerError::MalformedResponse), and
///   (like every ambiguity on this path) stops the submission rather than retrying it.
...
///   reconciliation required. Modelling it as required would turn the second case into a decode
///   error and lose the distinction the spec draws.
///
/// Unknown fields are ACCEPTED here, unlike the request types. The OpenAPI documents status codes
/// only there is no error-body schema at all — so this shape is inferred from the frozen fixture,
/// and refusing a field Circle happens to add would push an otherwise recoverable conflict into
/// reconciliation for no safety gain. The two fields that decide anything are still validated
/// newtypes.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
impl WithdrawConflict {
    ...
    /// defect, never a recovery and never a success.
    pub fn burn_tx_id(&self) -> &str {
...
/// be present-and-`null` (Circle documents a malformed Circle response is rejected, not acted
/// on).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
impl WithdrawalStatus {
    ...
    /// The burn-evidence field the submission was keyed on — and the key a 409 conflict is keyed
    /// on.
    pub fn burn_tx_id(&self) -> &str {
        self.burn_tx_id.as_str()
    }
...
/// The withdrawal status enum, verbatim: `created, verified, confirmed, finalized, expired,
/// failed`.
///
...
pub enum WithdrawalStatusKind {
    ...
    /// [`is_terminal`](Self::is_terminal) and reported through
...
impl WithdrawalStatusKind {
    ...
    /// failure). `expired` is deliberately **excluded**: the task and
    /// `TEST-AND-VERIFICATION-HARNESS.md` (`:191`-`:195`) define only `finalized`/`failed` as
    /// terminal, and treating `expired` as terminal would hand a resubmittable withdrawal to
    /// callers as though it were done. `expired`'s "stop polling" behavior is captured by
    /// [`is_retryable`](Self::is_retryable) instead.
