...
//!
//! It is a type of its own rather than a [`ListenerError`](crate::error::ListenerError) family, and
//! it lives beside the ledger rather than in `error.rs`, for the same reason
//! [`DecodeError`](crate::error::DecodeError) is its own type: a ledger failure is a statement
//! about ONE burn's bookkeeping, and the caller's response to it (stop, surface, let an operator
//! look) is not the response to a Circle transport failure. It is re-exported through
...
pub enum LedgerError {
    /// The configured ledger path is one SQLite would not have made durable — it would open
    /// cleanly, take every claim, and lose them all on the next restart: a cache wearing the
    /// ledger's name. The service does not start.
    ...
    /// one — silently misreading a burn's status is exactly the failure the ledger exists to
    ...
    /// The database could not be opened, read, or written (a missing parent directory, a
    /// permission, a full disk, a lock timeout). A listener that cannot reach its ledger must fail
    /// LOUDLY: running without one means re-submitting after the next restart.
    Store(Cause),

    /// A stored row cannot be read — an unknown status token, a timestamp that is not a timestamp.
    /// It is NEVER treated as absent: that would re-claim, and re-submit, a burn that may already
    /// be settled.
    ...
    /// for a burn with no claim behind it — the claim IS the decision, so a record without one
    /// means the decision was skipped.
    UnknownBurn { burn_tx_id: String },

    /// The machine has no `from → to` edge. The load-bearing case is `Submitted → Failed`: `Failed`
    /// is the re-claimable state, so that edge would let a retry driver re-POST a withdrawal Circle
    /// may already be releasing.
...
/// Every `rusqlite` failure becomes ONE ledger error, with the original preserved as its source —
/// the SQLite primary/extended code is what an operator needs, and a flattened string is not.
