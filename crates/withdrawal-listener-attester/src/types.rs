...
impl ProofStrength {
    /// The weaker of two strengths — a claim resting on several pieces of evidence is only as strong
...
/// **What** a piece of evidence proves — orthogonal to [`ProofStrength`], which says only how
/// strongly.
///
/// The two have to be separate, and the reason is the sharpest edge on this whole package. A
/// `GetNotesById` inclusion proof is genuinely CRYPTOGRAPHIC, and what it cryptographically proves is
/// that the note was **created**. It says nothing whatsoever about the note having been consumed.
/// "This note exists and was created in block N" is not "this burn happened" — and a package that
/// reported a strength without the scope would let the strongest label in the type system sit
/// silently next to the weakest claim, which is how a creation proof gets read as a confirmed burn
/// and native USDC is released against a note that is still sitting there unspent.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub enum ProvenFact {
    /// The note existed and was CREATED in the named block — what the `GetNotesById` inclusion proof
    /// gives (R-2), and the whole of what it gives.
    NoteCreatedInBlock,

    /// A node REPORTS that a transaction of the faucet's consumed the note (`SyncTransactions`,
    /// R-9/R-10). No inclusion proof accompanies it, and there is no `GetTransactionById` to check it
    /// against (R-8).
    NoteConsumedByTransaction,

    /// A node REPORTS the note's nullifier spent (`SyncNullifiers`, R-5). No inclusion proof, and
...
impl ProvenFact {
    ...
    ///
    /// Every consumption claim Miden can make today is a node report. That is the substance of
    /// `DEV-7` and it is why this predicate exists: it is what lets the package assert, mechanically,
    /// that no element is ever both CRYPTOGRAPHIC and a claim that a burn occurred.
...
/// "every element" can be written — and checked — as a rule about every element, rather than as four
/// assertions someone has to remember to extend.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub struct EvidenceElement {
    /// The element's name as `DC-8` writes it.
    pub name: &'static str,
...
/// cryptographic elements prove the note's CREATION; every element that speaks to the burn having
/// happened is NODE-TRUSTED ([`Self::consumption_trust`]) — `INV-BURN-EVIDENCE-TRUST`,
/// "retrievable ≠ cryptographically proved". Nor does a package mean the withdrawal succeeded: only
/// Circle's own terminal `finalized` says that
...
/// ambiguous reads is refused rather than rounded up —
...
impl EvidencePackage {
    /// **`pub(crate)`: a package is minted by [`assemble_evidence`](crate::evidence::assemble_evidence)
    /// and by nothing else.** It is the difference between a token that MEANS the evidence was read
    /// and checked, and a token that merely LOOKS like one.
    ///
    ...
    /// constructor would let a caller skip all of it and produce a value that is indistinguishable
    /// from an assembled one — same type, same labels, and a `consumption_trust()` of NODE-TRUSTED
    /// reporting on consumption evidence nobody ever read. The four fields would be whatever the
    /// caller typed, and Circle would release native USDC against them.
    ///
    /// So it is sealed. Outside this crate, holding an `EvidencePackage` is proof the checks ran; the
    /// only way to obtain one is to pass a [`BurnEvidenceReads`](crate::evidence::BurnEvidenceReads)
    /// port and have it answer consistently.
    ///
    /// The honest limit, stated rather than glossed: in-crate code can still call this. In-crate there
    /// is exactly one caller — the assembler — and that is pinned mechanically by
    /// `tests/evidence_structural_absence.rs`. A future verified upgrade path (the full-block route,
    /// `IMPL-FULLBLOCK-PATH`) would mint through here too, after running ITS checks.
    pub(crate) fn new(
        burn_tx_id: String,
        note_id: [u8; 32],
        nullifier: [u8; 32],
    ...
    /// (R-2). It proves the note was CREATED there ([`Self::note_id_proves`]); it is not evidence of
    /// a burn.
    pub fn note_id_strength(&self) -> ProofStrength {
        ProofStrength::Cryptographic
    }

    /// CRYPTOGRAPHIC — via that same note inclusion proof, and the block it names is the one the note
    /// was CREATED in.
    ...
    /// Every element of the package, in `DC-8` order, each with its strength and the fact it proves.
    ...
    /// wrong by eye: the package's *strongest* label is CRYPTOGRAPHIC, and that label belongs to the
    /// note's creation. The burn claim rests entirely on the two node reports behind it, so it is
    /// derived from their labels rather than written down as a constant — the day the optional
    /// full-block path (`IMPL-FULLBLOCK-PATH`, OPEN) upgrades the tx-linkage, this answer moves with
    /// it instead of being a promise someone forgot to revisit.
