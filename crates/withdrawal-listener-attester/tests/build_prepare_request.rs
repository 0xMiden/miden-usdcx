//! `T-LA-05` — build the `DC-9` `PrepareWithdrawalRequest` (the API JSON the partner authors) and
//! the `remoteDepositor` vs `sourceDepositor` distinctness (anti-`ASG-15`).
//!
...
//! (that is the B5 gate, `validate.rs`, T-LA-06). The oracle is deliberately non-vacuous: the
//! serialized JSON is asserted at the STRING level (the `sourceDepositor` key is absent as text, not
...
//! * `sourceDepositor` is structurally AND textually absent (`INV-REMOTEDEPOSITOR-VS-SOURCEDEPOSITOR`);
...
/// The §10.3 burn payload the burn note wrote — `(amount, destDomain, destRecipient, salt)`.
...
/// `0x` + lowercase hex of 32 bytes — the wire rendering the builder must produce for `DC-6`.
...
// ================================================================================================
// HAPPY PATH — every field mapped per the DC-9 table
// ================================================================================================
...
fn happy_path_maps_every_field() {
    ...
    // useCircleForwarding is a concrete bool (Q-DOM-2 OPEN → no forwarding invented).
...
/// The golden `DC-6` mapping: `remoteDepositor` is `account_id_to_bytes32(sender)` rendered 0x-hex,
/// and it matches the OpenAPI's `^0x[a-fA-F0-9]{64}$`.
#[test]
...
/// A supplied `salt` comes from the burn payload, so it renders 0x-hex 32B and is NOT a fresh random.
#[test]
...
/// the total debited on Miden; the smallest-unit⇄decimal scale is DEV-5, OPEN, so the amount is
...
/// `SchemaError::ValueXor`. (The builder used by `build_prepare_request` always sets exactly one, so
/// this pins the rule at its single owner.)
#[test]
...
// ================================================================================================
// sourceDepositor DISTINCTNESS (anti-ASG-15) — string-level, not only struct-shape
// ================================================================================================

/// The serialized request carries NO `sourceDepositor` key as TEXT — Circle fills it server-side
/// (`Q-DOM-3`). This is the non-vacuity oracle: a struct with no such field could still, in
...
fn serialized_request_has_no_source_depositor_key() {
    ...
    // and remoteDepositor IS present, carrying the DC-6 hex — the swap's positive control.
    assert!(
...
/// (`encoded`, `messageHashToSign`, a nested `spec`, `sourceSigner`, `version`) is emitted. Foreclosing
...
/// `0` placeholder (`Q-DOM-1` OPEN), so a request built against it is refused rather than shipped
/// with an out-of-range domain.
#[test]
