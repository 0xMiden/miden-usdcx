//! The Circle-facing half: the HTTP transport and everything that turns a Circle response into
...
//! | [`auth`] | the auth posture (the scheme is still Circle's to confirm): an injection point, not a scheme |
//! | [`rate`] | the rate governor (Circle's documented ceilings: 5 QPS/IP, 35 QPS global) |
//! | [`retry`] | exponential backoff + the never-drop and alert obligations |
...
//! | [`attestation_fetch`] | the three fetch shapes (by message hash, by tx hash, the batch poll) |
//! | [`info`] | `GET /v1/info` discovery |
//!
//! This half NEVER touches Miden. It produces a validated triple (payload + attestation + the
//! candidate pubkey the config holds) or a rejection, and the seam — the idempotency store — is
//! what a validated triple must cross before the Miden half sees it. Nothing here decides whether a
//! mint is AUTHORIZED: the ECDSA verify, the attester allowlist, the nonce, and the amount
//! reduction are all on-chain, which is why a bug in this half can withhold a mint but never cause
//! one.
//!
//! **Mock-only, by design.** Every live Circle leg is still `REQUIRES CIRCLE CONFIRMATION` — the
//! credential scheme, the Miden domain id Circle will assign, and the shape of the info response — so
