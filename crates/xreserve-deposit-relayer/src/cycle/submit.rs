...
//! for v0.16**. So the leg cannot be written here, and the two dishonest alternatives are worse than
//! not writing it: a stub that returns success would report mints that never happened, and a
...
//! not be faked for final acceptance (§11).
//!
//! So the leg is a **trait with no production implementation**. The orchestration composes against
//! it and is fully exercised; the adapter lands in R6, against a real client, proven by the GATING
//! real-local-node rows (T-RLY-14 / T-RLY-15). Until then
//! [`production_submit_port`] refuses, by name, and `main` fails at startup on it. That is the same
//! seam discipline the withdrawal track used (W8 defined the `BurnEvidenceReads` port; W9 composed
//! against it; W10 implemented it) — and `miden-client` is deliberately absent from this crate's
//! dependencies, so nothing here can quietly acquire one.
//!
...
//! `TransactionRequestBuilder` + `submit_new_transaction` need and nothing else — and answers with a
//! transaction id, an already-minted verdict, or one of exactly two errors. R6 implements it by
//! mapping the client's real answers onto those; it does not have to reshape the interface to do it.
//!
//! # The two errors are the orchestration's contract, not a taxonomy
//!
//! [`RelayerError::TransientSubmit`] retries and [`RelayerError::FatalSubmit`] does not — that split
//! is the whole reason the port returns typed errors rather than a `Box<dyn Error>`. WHICH real
//! failure is which is R6's judgement against a real node; that a node's sync lag must not strand a
//! deposit, and that a permanent refusal must not loop forever, is decided here (§8.4).
...
/// sender as its producer (unit-04's `XReserveMintNote::create` takes it), and submitting it from a
/// different account would produce a transaction whose output note is not the note that was built.
/// Constructed only by the orchestration, so no caller can pair a note with a stranger.
...
impl<'a> MintSubmission<'a> {
    ...
    /// The `XReserveMintNote` to submit. Every byte of it is unit-04's
    /// (`XReserveMintNote::create`); an adapter transports it and reads nothing out of it.
...
pub enum MintSubmitted {
    ...
    Accepted(TxId),
    /// The on-chain `usedNonces` assert-then-set refused it (D5c): this deposit is already minted,
    /// by an earlier attempt or a competing relayer (§8.2).
    ///
...
/// **The seam.** Hands a built `XReserveMintNote` to a Miden node.
///
/// Implemented by R6 against a real `miden-client`, and by the test adapter the orchestration suites
/// drive (explicitly NON-GATING — it fakes no Miden behaviour; it answers this interface on a
/// script). There is no production implementation in this crate: see [`production_submit_port`].
///
/// The `Pin<Box<dyn Future>>` return (rather than `async fn`) is what makes the trait usable behind
/// `&dyn` — the same shape [`HttpTransport`](crate::circle::HttpTransport) uses, for the same reason.
///
/// # Errors
...
///
/// R6 replaces this body with a real `miden-client`-backed adapter once v0.16 has a release. Until
...
/// attestations, claim nonces, build notes, and mint nothing — and would look, in every log and every
/// metric except the chain's, exactly like a relayer that was working.
///
/// # Errors
...
/// rather than the node answering. It is a TRANSIENT failure — a hung or slow node may recover — and
/// the deadline is what keeps one attempt from waiting forever, so the whole retry loop is bounded (the
/// envelope `RecoveryPolicy` validates against). A real `std::error::Error`, so it sits in the `Cause`
/// source chain exactly like a real adapter's timeout would.
#[derive(Debug)]
