...
//! walks the `Link`-paginated stream, the envelope binds `messageHash == keccak256(payload)` and
//! the DepositIntent decodes. Everything downstream is *action*: a mint note is built and a Miden
//! transaction is submitted. The seam is what stops an observation seen twice from becoming an
//! action taken twice, and what stops a restart from losing the place it had reached.
//!
//! It holds exactly two things:
//!
//! * the **submitted-nonce log** — one [`IdempotencyRecord`] per DepositIntent `nonce` (header
//!   field 9), carrying the attestation's `messageHash`, the Miden transaction the mint went out
//!   in, the block it committed in, and where it sits in the [`SubmissionStatus`] machine;
...
//!
//! The authoritative defence against a double mint is on-chain: the faucet's mint policy asserts
//! the deposit's nonce is absent from its `usedNonces` map and marks it used, in the same
//! transaction that mints — the replay guard on the faucet's only supply-increasing path. That
//! assert is what makes a second mint *impossible*. This store makes a second mint *attempt* not
//! happen — a different, weaker, still worthwhile job: a duplicate attempt burns a transaction on
//! an assert that must fail, and a lost cursor re-walks (or skips) a window of attestations.
//!
//! So a bug here can **withhold** a mint (a nonce parked in a status nothing retries) or **waste**
//! a transaction (an attempt the chain rejects). It cannot authorize one. Every decision below
//! follows from preferring the wasted transaction to the withheld mint — most visibly in
...
//! side. [`IdempotencyStore::is_nonce_submitted`] remains as a *read* (an operator query, a metric,
//! a pre-flight filter); it is not, on its own, a dedup.
//!
...
//! attestations on the same crash — a silently withheld mint, the one thing the no-silent-drops
//! rule forbids.
//!
//! Because the cursor only ever moves forward, a mint that keeps failing cannot be rediscovered by
//! polling: its page is behind us. [`IdempotencyStore::retryable`] is the work list that closes
//! that loop — the failed records, oldest first, each carrying the `messageHash` its attestation
//! can be re-fetched by.
//!
//! # Persistence: SQLite
//!
...
//! name. [`IdempotencyStore::open`] therefore refuses the reserved names outright and, after
//! opening, asks SQLite where the database actually landed: a database with no file behind it is
