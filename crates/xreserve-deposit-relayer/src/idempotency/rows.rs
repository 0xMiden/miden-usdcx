...
//! by a partial upgrade, or by a hand-edited `UPDATE` — so every field is CHECKED on the way out
//! (width, status token, integer range) and a row that does not make sense becomes
...
/// Writes `current` forward into `to`, with the transaction id / block number the transition carries.
/// The CALLER has already checked the machine's edge and is inside the write transaction.
///
...
/// A 32-byte digest column, width-checked. (The schema `CHECK`s the width too — this is the reader's
/// own guard, because the row may have been written by something that is not this build.)
...
/// SQLite integers are signed 64-bit; the store's seconds are unsigned. Both conversions are checked
...
/// Every `rusqlite` failure becomes ONE relayer error, with the original preserved as its source (the
/// SQLite primary/extended code is what an operator needs; a flattened string is not).
