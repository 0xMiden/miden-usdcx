...
fn t_rly_20_accept_canonical_raw_keccak_vectors() {
    for v in att_vectors() {
        ...
        let (bound, bound_digest) =
            verify_message_hash(&with_0x(&hex_of(&payload)), &with_0x(&hex_of(&digest)))
                .unwrap_or_else(|e| {
                    panic!("{}: canonical raw-keccak vector must bind, got {e}", v.id)
                });
        assert_eq!(
            bound_digest.to_vec(),
            digest,
            "{}: the verified digest is the one that was bound",
            v.id
        );
...
fn t_rly_20_accept_partner_fixture(#[case] prefixed: bool, #[case] upper: bool) {
    ...
    let (bound, _digest) = verify_message_hash(&render(tv.payload()), &render(&tv.message_hash()))
...
fn t_rly_20_fixture_signs_raw_keccak_for_arbitrary_payloads() {
    ...
    for id in [
        ...
        assert_eq!(
            verify_message_hash(&tv.payload_hex(), &tv.message_hash_hex())
                .expect("binds")
                .0,
            payload
        );
...
fn t_rly_06_foreign_key_signature_passes_shape_check() {
    ...
    assert_eq!(
        verify_message_hash(&att.payload_hex(), &att.message_hash_hex())
            .expect("binds")
            .0,
        payload,
