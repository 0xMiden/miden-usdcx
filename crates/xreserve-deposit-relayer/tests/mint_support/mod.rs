...
//! layout the shared encoding crate owns — the note's storage packing, its attachment content, its
//! script root and its tag are all read back through the shared encoding crate's / the protocol's
//! own API in the tests themselves.
//!
//! The attestation vectors come from [`crate::fixtures`] — the one partner-attester fixture every
//! relayer slice shares (a real secp256k1 key over a real keccak digest of a canonical
//! DepositIntent payload), so the mint-note suite cannot drift onto a second, divergent key.
...
/// OS entropy; determinism here is a property of the test, not of the builder (which is generic
/// over the `FeltRng`).
...
/// [`AttestationObject`], and run through [`ValidatedAttestation::validate`] — the same the
/// documented policy checks (raw-keccak binding + the 65-byte shape) the Circle client runs. A test
/// never hands the builder bytes by any other route: that IS the "no raw-bytes side door"
/// requirement, expressed as the only way the suite can construct an input.
...
/// the BINDING, never the DepositIntent structure, so a payload Circle signed but the shared
/// encoding crate's codec refuses can and does reach the builder. It must surface as a typed error,
/// not a panic and not a malformed note.
...
/// the structural-reject vectors alike — the ONE artifact that drives the shared encoding crate's
