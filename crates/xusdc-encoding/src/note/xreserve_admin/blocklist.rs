...
//! about it is faucet-specific and nothing here reimplements it.
//!
...
//! The standard block procedure validates nothing about its target, so this factory refuses to
//! build such a note in the first place.
//!
//! That refusal is a guard against operator error, not an authorization boundary. The blocklist
//! administrator holds the role and can assemble the standard note directly, past this factory —
...
/// Builds the faucet's transfer-blocklist admin notes over the standard blocklist config note.
///
...
pub struct XReserveBlocklistConfigNote;

impl XReserveBlocklistConfigNote {
    /// The standard blocklist config note script.
    pub fn script() -> NoteScript {
        BlocklistConfigNote::script()
    }

    /// The standard blocklist config note script root — the faucet's single allowlisted blocklist
    /// admin entry.
    pub fn script_root() -> NoteScriptRoot {
        BlocklistConfigNote::script_root()
    }

    /// Builds a note that adds `account` to `faucet_id`'s transfer blocklist.
    ///
    /// # Errors
    ///
    ...
    /// Builds a note that removes `account` from `faucet_id`'s transfer blocklist.
    ///
    /// Unblocking the faucet itself is permitted — it is the recovery path from a self-block that
    /// was assembled past this factory.
    ///
    /// # Errors
    ///
    ...
    /// Assembles the standard note for `config`, tagged for the faucet and sent by `sender`.
    fn build<R: FeltRng>(
        sender: AccountId,
        faucet_id: AccountId,
