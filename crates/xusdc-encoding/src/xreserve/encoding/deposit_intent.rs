//! DepositIntent layout + parse family, frozen signatures per `COMPONENT-SPEC.md §6.5`
//! (INV-DEPOSITINTENT-PARSE; DC-1). Implemented (routine R4) per the frozen §8.1
//! validation order — this library owns the STRUCTURAL checks only (magic, version,
//! truncation, length relation, amount/localToken/localDepositor nonzero); the
//! domain/identifier equality compares are faucet-owned (D5a steps 5–6) and out of scope.
//! The hookData cap is `REQUIRES CIRCLE CONFIRMATION` (DEV-6) — the 1024-felt
//! NoteStorage bound is the documented default, not a resolution.
...
/// DC-1 constants (CIRCLE-DATA-SCHEMAS.md:21-35 via the frozen 04 spec; C-10: the
/// 240-byte header packs to 60 u32-LE felts, 4 bytes per felt — NOT 240/8).
...
/// The NoteStorage felt bound (EL N-4 :100; the DEV-6 hookData cap default).
...
fn bytes32_at(bytes: &[u8], offset: usize) -> [u8; 32] {
    bytes[offset..offset + 32].try_into().expect("32-byte window")
}

/// Structural parse + the library-owned checks (truncation, magic, version, non-zero
/// fields, total-length relation — frozen §8.1 steps 8, 1–4, 7). Does NOT perform the
/// faucet-owned domain/identifier equality compares (D5a steps 5–6).
pub fn parse_deposit_intent_header(bytes: &[u8]) -> Result<DepositIntentHeader, EncodingError> {
    ...
    let magic = be_u32(bytes, deposit_intent_field_offset(DepositIntentField::Magic));
    if magic != DEPOSIT_INTENT_MAGIC {
        return Err(EncodingError::BadMagic);
    }

    let version = be_u32(bytes, deposit_intent_field_offset(DepositIntentField::Version));
    if version != DEPOSIT_INTENT_VERSION {
        return Err(EncodingError::BadVersion);
    }

    let amount = bytes32_at(bytes, deposit_intent_field_offset(DepositIntentField::Amount));
    if amount.iter().all(|&b| b == 0) {
        return Err(EncodingError::ZeroField { field: DepositIntentField::Amount });
    }

    let local_token =
        bytes32_at(bytes, deposit_intent_field_offset(DepositIntentField::LocalToken));
    if local_token.iter().all(|&b| b == 0) {
        return Err(EncodingError::ZeroField { field: DepositIntentField::LocalToken });
    }

    let local_depositor =
        bytes32_at(bytes, deposit_intent_field_offset(DepositIntentField::LocalDepositor));
    if local_depositor.iter().all(|&b| b == 0) {
        return Err(EncodingError::ZeroField { field: DepositIntentField::LocalDepositor });
    }

    let hook_data_len =
        be_u32(bytes, deposit_intent_field_offset(DepositIntentField::HookDataLen));
    ...
    Ok(DepositIntentHeader {
        ...
        version,
        amount,
        remote_domain: be_u32(bytes, deposit_intent_field_offset(DepositIntentField::RemoteDomain)),
        ...
        local_token,
        local_depositor,
        max_fee: bytes32_at(bytes, deposit_intent_field_offset(DepositIntentField::MaxFee)),
        nonce: bytes32_at(bytes, deposit_intent_field_offset(DepositIntentField::Nonce)),
        hook_data_len,
    })
}

/// The u32-LE-packed on-chain preimage: 60 felts for the header plus ceil(hookDataLen/4)
/// felts of hookData (C-10 packing — the same `bytes_to_packed_u32_elements` primitive,
/// E-13). Validates the structure first, then errors `HookDataTooLarge` past the
/// 1024-felt NoteStorage bound (DEV-6 default).
...
// TESTS — TV-DI-1..9 (frozen 04 TEST-AND-VERIFICATION-HARNESS §2.4)
// ================================================================================================
...
mod tests {
    ...
    fn tv_di_1_positive_parse() {
        ...
        for vec in v.families.di.iter().filter(|v| v.kind == "accept") {
            ...
            assert_eq!(h.remote_domain, f.remote_domain, "vector {}: remoteDomain", vec.id);
            assert_eq!(h.hook_data_len, f.hook_data_len, "vector {}: hookDataLen", vec.id);
            assert_eq!(h.amount, f.bytes32("amount"), "vector {}: amount", vec.id);
            assert_eq!(h.remote_token, f.bytes32("remote_token"), "vector {}: remoteToken", vec.id);
            assert_eq!(
                h.remote_recipient,
            ...
            assert_eq!(h.local_token, f.bytes32("local_token"), "vector {}: localToken", vec.id);
            assert_eq!(
                h.local_depositor,
    ...
    fn tv_di_rejects(#[case] id: &str) {
        ...
        match vec.expected_variant.as_deref() {
            ...
            Some("ZeroField:LocalDepositor") => assert_matches!(
                result,
                Err(EncodingError::ZeroField { field: DepositIntentField::LocalDepositor }),
                "{id}"
            ),
            Some("LengthMismatch") => {
    ...
    fn tv_di_7_sixty_felts_and_1024_bound() {
        ...
        for vec in v.families.di.iter().filter(|v| v.kind == "accept") {
            ...
            assert_eq!(&felts[..60], &expected[..60], "vector {}: 60-felt header", vec.id);
            assert_eq!(felts, expected, "vector {}: full preimage", vec.id);
            assert!(felts.len() <= 1024, "vector {}: NoteStorage bound (N-4)", vec.id);
        }
        let overflow =
            v.families.di.iter().find(|v| v.id == "di-rej-hookdata-overflow").expect("vector");
        assert_matches!(
    ...
    /// bytes are unchanged afterwards (note-input immutability analogue, C-12).
    #[test]
    ...
    /// (CIRCLE-DATA-SCHEMAS.md:21-32 via the frozen 04 spec DC-1 table).
    #[test]
    fn tv_di_9_offsets_table() {
        ...
        for (field, off) in expected {
            assert_eq!(deposit_intent_field_offset(field), off, "DC-1 offset of {field:?}");
        }
    }
}
