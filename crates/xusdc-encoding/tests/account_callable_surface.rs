...
///   `mint_policy::check_policy` (the ACTIVE mint policy); the shared-encoding/parser/attestation
///   procs are `exec`-only helpers off the account interface, and pause and blocklist
///   administration are STOCK now (the manager rows below);
