...
//! - S3 admin bring-up: attester, max_supply, and min_burn, each with its
//!   non-administrator reject;
...
async fn assembled_faucet_full_lifecycle() -> Result<()> {
    ...
    let mut pf = setup_production_faucet(MAX_SUPPLY, 0, |recipient, faucet_id| {
        ...
        vec![
            ...
            // 4: S3c stranger minimum-burn update (reject)
            stock_set_min_burn_amount_note(stranger(), route, MIN_BURN, &mut note_rng(917))
                .expect("building the seeded min-stranger note"),
            // 5: S3c administrator minimum-burn update
            stock_set_min_burn_amount_note(administrator(), route, MIN_BURN, &mut note_rng(918))
    ...
    // S3c: the administrator sets the min-burn floor through the standard configuration note; a
    // stranger's attempt is rejected.
    ...
    let tx = consume_committed_note(&pf.mock_chain, &faucet, note_id(5))
        .await
        .expect("S3c: the administrator's minimum-burn update must succeed");
    commit(&mut pf.mock_chain, &tx)?;
    let faucet = committed(&pf.mock_chain, faucet_id)?;
    assert_eq!(
