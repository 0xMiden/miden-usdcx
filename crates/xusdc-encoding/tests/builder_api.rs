...
/// [faucet, Pausable, policy manager, xreserve, MinBurnAmount, BasicBlocklist, PausableManager,
...
fn production_composition_installs_one_xreserve_and_one_manager() -> Result<()> {
    ...
    assert!(
        components[5].component_code().as_package() == BasicBlocklist::code().as_package(),
        "component 5 must be the BasicBlocklist companion"
    );
    Ok(())
}
