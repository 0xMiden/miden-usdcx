...
//! faucet's entire authorization model is the FROZEN 8-root note allowlist — admitting the
...
//! vector those rows would need: the note-script allowlist stays the exact 8 ratified roots.
...
/// Source layer: the builder's single-source 8-root set does not name either forbidden root
...
fn config_note_root_is_not_in_the_builder_allowlist() {
    ...
    for (name, root) in forbidden_roots() {
        assert!(
            ...
            "the {name} script root must NOT be a member of the builder's 8-root note-script \
             allowlist"
        );
    }
}
...
/// in its allowlist slot, and the slot holds EXACTLY 8 keys — the composition went through
...
fn config_note_root_is_not_in_the_auth_components_allowlist() -> Result<()> {
    ...
    assert_eq!(
        note_keys.len(),
        8,
        "the auth component's note-script allowlist must hold EXACTLY the 8 ratified roots; \
         found {} (a 9th/10th key means a force-inserting constructor was used)",
        note_keys.len(),
    );
    for (name, root) in forbidden_roots() {
        assert!(
...
/// On-chain layer: the BUILT production faucet's materialized allowlist storage holds exactly 8
...
fn config_note_root_is_absent_from_the_built_accounts_allowlist() -> Result<()> {
    ...
    assert_eq!(
        roots.len(),
        8,
        "the built faucet's on-chain note-script allowlist must hold EXACTLY the 8 ratified \
         roots; found {}",
        roots.len(),
    );
    for (name, root) in forbidden_roots() {
