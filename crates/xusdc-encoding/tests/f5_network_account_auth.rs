...
//! note carries two attachments — the merged transport (the attestation count prefix, the
//! attestation, and the deposit intent) and the target routing the note to the faucet with an
...
use xusdc_encoding::note::xreserve_mint::{
    MintAttestation, XUsdcMintNote, XUSDC_MINT_ATTESTATION_COUNT, XUSDC_MINT_ATTESTATION_NUM_WORDS,
...
/// attachments — the merged scheme-4 transport (a one-felt attestation count padded to a word,
/// then the 11-word attestation `[feeAmount(8), pubkey(16), signature(17), pad(3)]`, then the
/// packed DepositIntent preimage) and the scheme-2 `NetworkAccountTarget` routing attachment
...
fn mint_note_carries_the_merged_transport_and_the_routing_target() -> Result<()> {
    ...
    // the merged content, at the documented offsets: count prefix, attestation, intent
    let transport = note
        .attachments()
        .iter()
    ...
    expected.push(Felt::from(XUSDC_MINT_ATTESTATION_COUNT));
    ...
    assert_eq!(
        ...
        "the prefix ({XUSDC_MINT_TRANSPORT_PREFIX_WORDS} word) + attestation          ({XUSDC_MINT_ATTESTATION_NUM_WORDS} words) sections precede the intent"
    );
    expected.extend(intent);
    assert_eq!(
        transport, expected,
        "the merged transport attachment is [count, 0, 0, 0] || attestation(44) || padded intent",
    );
    assert_eq!(
        usize::from(
            note.attachments()
        ...
        expected.len() / 4,
        "the committed word count is exactly prefix + attestation + padded intent",
    );
