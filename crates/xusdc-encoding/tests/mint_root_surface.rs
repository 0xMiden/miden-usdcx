...
/// procedure-role map. `attestation_verify::pubkey_commitment` is also no longer a callable root —
/// #47 made it an exec-only export (see `FROZEN_EXEC_ONLY_EXPORTS` below). There is deliberately
/// NO custom transport/burn/config root
