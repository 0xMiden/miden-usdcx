...
//! administrator's account, but account-bound; it is the faucet's only authority handle. This file covers the
//! administrator
...
//! ERR_MAX_SUPPLY_NOT_MUTABLE, even for the administrator). Stock `set_max_supply` is reused verbatim — no new MASM.
...
// DOM role-holders DOM_PAUSER = id(2) / DOM_MANAGER = id(3) — privileged non-administrators the administrator-ONLY proof
// rejects.
...
/// On an IMMUTABLE faucet, even the administrator's `set_max_supply` traps the EXACT ERR_MAX_SUPPLY_NOT_MUTABLE —
...
async fn set_max_supply_immutable_traps() -> Result<()> {
    ...
    let result = run_set_max_supply_tx(&harness, &account, administrator(), 500_000, 7).await;
...
async fn set_max_supply_owner_succeeds() -> Result<()> {
    ...
    let executed = run_set_max_supply_tx(&gm.harness, &account, administrator(), 500_000, 7)
        .await
        .expect("the administrator's set_max_supply must succeed on a mutable faucet");
...
async fn assert_set_max_supply_non_owner_rejected(sender: AccountId) -> Result<()> {
    ...
    assert_eq!(
        ...
        before,
        "a rejected non-administrator set_max_supply leaves token_config unchanged"
    );
    Ok(())
}
...
async fn set_max_supply_below_supply_rejects() -> Result<()> {
    ...
    let account = faucet_account(&gm.harness);
    let result = run_set_max_supply_tx(&gm.harness, &account, administrator(), 300_000, 7).await;
...
/// surface in the Domain-Pauser-only model), an administrator-sent `set_max_supply` passes mutability + auth
/// but traps the EXACT
...
async fn set_max_supply_paused_rejects() -> Result<()> {
    ...
    // tx2: set_max_supply by the administrator now traps the EXACT pause error (mutability + auth pass).
    let result = run_set_max_supply_tx(&gm.harness, &evolved, administrator(), 500_000, 7).await;
