...
/// (the amount witness the scale-0 shell proves), and `exec`s the faucet `assert_mint_amounts`
/// shell. The proc returns `[]`, so the staged-then-consumed stack restores the 16-depth `call`
/// boundary.
