...
pub fn grounding_components() -> Vec<AccountComponent> {
    let rbac = RoleBasedAccessControl::builder()
        .role(RoleConfig::new(RoleBasedAccessControl::admin_role()).with_member(admin_holder()))
        .role(RoleConfig::new(pauser_symbol()).with_member(pauser_holder()))
        .role(RoleConfig::new(blocklist_symbol()).with_member(blocklist_holder()))
        .build()
        .expect("the grounding RBAC seed is valid");
    vec![
