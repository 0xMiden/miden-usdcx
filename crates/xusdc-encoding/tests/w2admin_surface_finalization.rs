...
/// The callable surface is the ratified 65 roots, and not one of them is an administratorship procedure.
/// The count is asserted against the real composition, so a stale removal shows up here.
#[test]
fn the_callable_surface_is_sixty_five_roots_and_carries_no_ownership_row() -> Result<()> {
