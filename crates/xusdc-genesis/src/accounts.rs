...
pub fn build_faucet(config: &GenesisToolConfig) -> Result<Account> {
    ...
    XReserveStablecoinBuilder::builder()
        ...
        .owners(config.accounts.owner.clone())
        .attest_admin_holders(config.accounts.attest_admin.clone())
        .pauser_holders(config.accounts.pauser.clone())
        .unpauser_holders(config.accounts.unpauser.clone())
        .blocklist_manager_holders(config.accounts.blocklist_manager.clone())
