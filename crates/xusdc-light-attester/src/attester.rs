...
pub enum StartError {
    ...
    StoreLocked,
    /// Re-pinning is deliberately not automatic. A future controlled maintenance path must verify
    /// the replacement anchor independently and reconcile already-submitted withdrawals first.
    ...
    FaucetMissing,
    #[error("configured trusted anchor is not the requested, self-consistent pinned block")]
    TrustedAnchorInvalid,
