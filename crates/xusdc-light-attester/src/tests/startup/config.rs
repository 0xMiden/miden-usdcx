...
use crate::config::{Cli, MidenNetwork};
...
fn cli_surface_is_explicit() {
    ...
    assert_eq!(valid.load().miden_network(), MidenNetwork::Devnet);
    let mut testnet = valid.clone();
    testnet.replace("--miden-network", "testnet");
    assert_eq!(testnet.load().miden_network(), MidenNetwork::Testnet);
    ...
    let mut unsupported_network = valid.clone();
    unsupported_network.replace("--miden-network", "mainnet");
    assert_cli_error(&unsupported_network, ErrorKind::InvalidValue);
