...
pub(crate) enum VerifyError {
    ...
    UnknownSalt,
    #[error("Circle returned the same burn more than once")]
    DuplicateSalt,
    #[error("the salt matches more than one burn")]
    AmbiguousSalt,
...
impl VerifiedWithdrawal {
    pub(crate) fn note_ids(&self) -> impl Iterator<Item = NoteId> + '_ {
        self.batches.iter().map(|batch| batch.note_id)
    }
}
...
pub(crate) fn verify_prepared_response(
    ...
    if response.batches.len() != burns.len() {
    ...
    let mut seen = vec![false; burns.len()];
    let mut batches = Vec::with_capacity(burns.len());
    for batch in response.batches {
        ...
        // Salt is the note serial sent by prepare. Never choose the first of several matches.
        let mut matches = burns.iter().enumerate().filter(|(_, burn)| {
            B256::from(burn.burn.note.as_note().serial_num().as_bytes()) == spec.salt
        });
        let (index, burn) = matches.next().ok_or(VerifyError::UnknownSalt)?;
        if matches.next().is_some() {
            return Err(VerifyError::AmbiguousSalt);
        }
        if seen[index] {
            return Err(VerifyError::DuplicateSalt);
        }
        seen[index] = true;
