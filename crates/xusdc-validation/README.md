...
- **Pins:** protocol family crates.io `=0.16.0-alpha.4`, `miden-client` /
  ...
  [`VALIDATION-RECORD-LNV5.md`](VALIDATION-RECORD-LNV5.md) (LNV-5, generated by the gate run).
...
- **LNV-5:** THE consolidated §11.2 gate run — the WHOLE A–L matrix on ONE fresh node (the
  ...
  every warning triaged). Generates the VALIDATION RECORD + the three evidence packets
  (`LNV5-F7-EVIDENCE-PACKET.md`, `LNV5-NTX-LIVENESS-VERDICT.md`,
  `LNV5-BURN-GETNOTESBYID-CAPTURE.hex`) deterministically from the run. **The gate PASS is a
  HUMAN decision — the binary never declares it.**
...
# writes VALIDATION-RECORD-LNV5.md + the three evidence packets + evidence-lnv5.json
