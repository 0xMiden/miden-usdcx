...
// MASM: `deposit_intent_parser.masm`, `burn_policy.masm`, `xreserve_mint.masm`, `pause_admin.masm`,
// the owner/role gates, and the stock note/tx-script allowlist primitives). A reject that does not
// carry ITS error is not the gate the row proves — the assertion rejects it.
// ================================================================================================
...
/// R-BURN-2: the burn amount is below the configured minimum burn size.
pub const ERR_BURN_BELOW_MIN: &str = "burn amount is below the minimum burn size";
/// R-MINT (`ERR_XRESERVE_SUPPLY_CAP`): the mint amount exceeds the faucet supply cap.
pub const ERR_SUPPLY_CAP: &str = "mint amount exceeds the faucet supply cap";
...
fn assert_rejected_with(v: &Verdict, expected: &str, ctx: &str) -> Result<()> {
    match v {
        Verdict::Accepted => {
            bail!("{ctx}: expected a REJECT carrying '{expected}', but the consumption was ACCEPTED")
        }
...
fn assert_word(actual: Word4, expected: Word4, ctx: &str) -> Result<()> {
    ensure!(actual == expected, "{ctx}: read-back word mismatch (got {actual:?}, expected {expected:?})");
    Ok(())
}
...
pub fn assert_c1(o: &C1SetAttester) -> Result<()> {
    assert_word(o.a_marker_after_enable, MARKER_SET, "C1: attester A after set_attester(A, enabled=1)")?;
    assert_word(o.a_marker_after_rotate, MARKER_CLEAR, "C1: attester A after rotation (must be disabled)")?;
    assert_word(o.b_marker_after_rotate, MARKER_SET, "C1: attester B after rotation (must be enabled)")?;
    assert_rejected_with(&o.mint_by_a, ERR_ATTESTER_NOT_ALLOWLISTED, "C1: mint attested by the rotated-out A")?;
...
pub fn assert_c2(o: &C2MinBurn) -> Result<()> {
    ...
    ensure!(o.raised_min > o.lowered_min, "C2: the raised minimum must exceed the lowered minimum");
    assert_rejected_with(&o.burn_below_raised, ERR_BURN_BELOW_MIN, "C2: a burn below the raised minimum")?;
    ensure!(
    ...
    assert_accepted(&o.burn_at_lowered, "C2: a burn exactly at the lowered minimum")?;
    Ok(())
}
...
pub fn assert_c3(o: &C3MaxSupply) -> Result<()> {
    ...
    assert_rejected_with(&o.over_cap_mint, ERR_SUPPLY_CAP, "C3: a mint exceeding the supply cap")?;
    assert_accepted(&o.within_cap_mint, "C3: a mint within the supply cap (baseline)")?;
    Ok(())
}
...
pub fn assert_c4(o: &C4Pause) -> Result<()> {
    assert_word(o.is_paused_after_pause, MARKER_SET, "C4: is_paused after the DOM_PAUSER pause")?;
    ...
    assert_word(o.is_paused_after_unpause, MARKER_CLEAR, "C4: is_paused after the DOM_PAUSER unpause")?;
...
pub fn assert_c5(o: &C5RoleRotation) -> Result<()> {
    ...
    assert_rejected_with(&o.revoked_pauser_pause, ERR_LACKS_ROLE, "C5: a pause by the revoked account")?;
    Ok(())
}
...
pub fn assert_c6(rejects: &[AdminGateReject]) -> Result<()> {
    ensure!(!rejects.is_empty(), "C6: the negative set must be non-empty");
