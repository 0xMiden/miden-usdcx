...
pub struct CfRunEvidence {
    ...
    /// The full observation record (every verdict + committed read-back).
    pub observations: RowsCfObservations,
    /// Per-row PASS/FAIL verdicts.
    pub rows: Vec<CfRowVerdict>,
    /// Archived node logs (path + size + ERROR/WARN counts).
    pub logs: Vec<LogManifestEntry>,
}

/// Writes `<run_root>/evidence-cf.json` from the observations + per-row verdicts.
pub fn write_cf_evidence(
    cfg: &RunConfig,
    obs: &RowsCfObservations,
    verdicts: &[(&str, Result<()>)],
) -> Result<PathBuf> {
    let rows = verdicts
        .iter()
        .map(|(row, r)| CfRowVerdict {
            row: (*row).to_string(),
            pass: r.is_ok(),
