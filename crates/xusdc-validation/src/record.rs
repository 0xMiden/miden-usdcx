//! The consolidated LNV-5 gate artifacts: the per-row A + C–L verdict map, the generated
...
use crate::assertions::assert_row_a;
...
/// The matrix rows, in matrix order (rows C1–C6 aggregate under row C).
pub const MATRIX_ROW_IDS: [&str; 11] = ["A", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L"];

/// The matrix row titles (the row names, condensed).
const MATRIX_ROW_TITLES: [(&str, &str); 11] = [
    ("A", "deploy + recognize"),
    ("C", "admin suite (C1–C6)"),
    ("D", "mint happy path"),
    ("E", "mint negatives"),
...
/// The outcome list must be EXACTLY the matrix rows, in matrix order, each once — a
...
pub fn validate_row_outcomes(outcomes: &[RowOutcome]) -> Result<()> {
        ...
        "the validation matrix has {} rows, got {}",
        MATRIX_ROW_IDS.len(),
        outcomes.len()
    );
...
pub struct RecordContext {
    ...
    pub run_root: String,
    /// The row-A faucet (the deploy + recognize subject).
    pub ab_faucet_id: String,
    /// The row-A deploy transaction id and its commit block.
...
pub fn render_validation_record(
    ...
    s.push_str(
        "# LNV-5 Validation Record — the consolidated §11.2 full-matrix gate run (rows A + C–L)\n\n",
    );
    s.push_str(
        "> This LNV-5 matrix is the EXHAUSTIVE gate over every matrix row the faucet still has \
         (A + C–L); the pre-deploy confidence gate is the leaner `sanity_e2e` \
         (`VALIDATION-RECORD-SANITY.md`). Both run against the **v16 node** — `stack.rs` brings it \
         up via the client repo's `start-test-node.sh` (`MIDEN_V16_NODE_DIR`). The LNV-5 live rows \
         are `#[ignore]`d (operator-run).\n\n",
    );
    s.push_str(
        "One deterministic pass over the WHOLE validation matrix on ONE fresh local \
         `miden-node` v16 stack: deploy → admin → mint → burn → conservation (the LNV-1..4 \
    ...
    s.push_str(
        "- Pins (full toolchain/dep ledger: `VALIDATION-RECORD.md` §2): `miden-node` **v16** \
         ...
         **=0.16.0-rc.1** (crates.io); protocol family crates.io **=0.16.0-rc.4**.\n\n",
    );
    ...
    s.push_str(
        "Requirements: `MIDEN_V16_NODE_DIR` set to the v16 client repo (its `start-test-node.sh` brings up the node), loopback ports 57291 / 50101 / 50301 / 50051 \
         free. The command bootstraps a FRESH genesis, runs the whole matrix, tears the \
...
pub fn render_f7_packet(gj: &RowsGjObservations, main_commit: &str) -> String {
    ...
    format!(
         ...
         **This packet is EVIDENCE ONLY: it makes NO acceptability decision. DEV-7 stays OPEN — \
         a Circle-owned decision.** Produced on a fresh local `miden-node` v16 stack from \
         ...
         rejected by the v16 `miden-node` (captured verbatim above), the stock \
         `miden-client` cannot present the sequencer's `x-miden-network-tx-auth` header, \
