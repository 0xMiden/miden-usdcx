...
| `WAVE1-S1-RECOMPOSITION.md` | KEEP (new) | The Wave-1 S1 recomposition record (stock MintNote + attestation MintPolicy + build-seeded config + stock MinBurnAmount): the restated INV-MINT-SECURITY, the re-materialized 14-root allowlist + 64-root callable-surface ratification tables, the deletion ledger, the frozen-surface statement, and the routing-proof caveat for the v2 deploy. |
