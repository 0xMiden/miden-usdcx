...
| Doc | Disposition | Notes |
|---|---|---|
| `DECISION-DEC4-REVERSAL-IDENTIFIER-DERIVE.md` | KEEP (new) | The DEC-4-reversal decision record: the mint path DERIVES the faucet identifier from the native account id instead of reading a seeded slot, and the whole identifier-init mechanism (MASM module, note script, Rust factory, storage slot, allowlist root, callable procedure) is deleted. Carries the rationale (the own-id identifier is a fixpoint of the storage it would live in; the frozen `AccountId`→bytes32 encoding makes a parity-proven derivation possible), the byte-parity and behavioral evidence tables, and the human ratification tables for the re-materialized surfaces — the note-script allowlist **9 → 8** and the callable surface **62 → 61**. `Q-CRY-4` and `DEV-10` stay OPEN. |
| `DECISION-F4-REVERSAL-TRANSFER-BLOCKLIST.md` | KEEP (new) | The F4-reversal decision record: wiring the stock v0.16 `BasicBlocklist` as the active send+receive policy + the dedicated external `BLK_MANAGER` role. Carries the human ratification table as it stood at that record (14 note-script roots and a callable surface of 65 roots; both re-materialized since — W2-ADMIN took them to 12 and 75, the admin-surface finalization to 9 and 69, then the deposit-intent consolidation to 65 and the MASM-hygiene `@account_procedure` strip to **62** with the allowlist unchanged at 9, and the DEC-4 reversal then re-materialized to **61** with the allowlist at **8** (the mint path derives the faucet identifier from the account's own id, so `identifier_init` and its note are deleted)), the allowed-maps, the BLK_MANAGER role row + felt, the module-registration rows MOD-1/2, the Enabled-flag statement) and the follow-up checklist. Supersedes the (unmaterialized) `DECISION-F4-BASIC-ASSET-NO-TRANSFER-POLICY.md`; provenance is an in-repo summary of the adversarially-audited research pass (no outside path). |
| `WAVE1-S1-RECOMPOSITION.md` | KEEP (new) | The Wave-1 S1 recomposition record (stock MintNote + attestation MintPolicy + build-seeded config + stock MinBurnAmount): the restated INV-MINT-SECURITY, the allowlist + callable-surface ratification tables as they stood at that record (14 and 64 roots; both re-materialized since — W2-ADMIN took them to 12 and 75, the admin-surface finalization to 9 and 69, then the deposit-intent consolidation to 65 and the MASM-hygiene `@account_procedure` strip to **62** with the allowlist unchanged at 9, and the DEC-4 reversal then re-materialized to **61** with the allowlist at **8** (the mint path derives the faucet identifier from the account's own id, so `identifier_init` and its note are deleted)), the deletion ledger, the frozen-surface statement, and the routing-proof caveat for the v2 deploy. |
| `PLAN-W2-ADMIN-STANDARD-CONFIG-NOTES.md` | KEEP (new) | The W2-ADMIN integration plan and its ratified outcome: the stock `PausableManager`/`BlocklistManager` adoption, the flip to `Authority::RbacControlled` with its procedure-role map, and the self-block regression that came with the stock blocklist path. The counts it records (75 callable, 12 allowlist) are the composition AS OF that record; the admin-surface finalization re-materialized them to 69 and 9, and the callable surface has moved twice since — the deposit-intent consolidation to 65 and the MASM-hygiene `@account_procedure` strip to **62** with the allowlist staying 9, and the DEC-4 reversal then re-materialized to **61** with the allowlist at **8** (the mint path derives the faucet identifier from the account's own id, so `identifier_init` and its note are deleted). |
| `PLAN-W2-ADMIN-SURFACE-FINALIZATION.md` | KEEP (new) | The admin-surface-finalization plan and its ratified outcome: removing `Ownable2Step`, moving `identifier_init`'s gate onto the account-wide authority, and adopting the stock 4-selector `RbacActionNote`. Carries the capability enumeration for that note (the role graph becomes runtime-mutable and self-renounce becomes reachable), the surface/allowlist deltas to **69** and **9**, and the re-materialized-pin ledger. The callable surface has been re-materialized twice since: the deposit-intent consolidation took the parser's three assertion procedures and the shared `encoding::parse_deposit_intent` off the surface (69 → 65), and the MASM-hygiene pass then re-materialized to **62** (the three remaining exec-only xreserve helpers — `encoding::bytes32_to_key`, `encoding::verify_uint256_to_asset_amount` and `attestation_verify::verify_attestation` — dropped `@account_procedure`, leaving the three genuine entry points on the interface), and the DEC-4 reversal then re-materialized to **61** with the allowlist at **8** (the mint path derives the faucet identifier from the account's own id, so `identifier_init` and its note are deleted); see `DECISION-DEC4-REVERSAL-IDENTIFIER-DERIVE.md`. |
