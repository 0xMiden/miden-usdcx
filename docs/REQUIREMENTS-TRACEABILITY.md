...
| IMPL-DEV-1 | The sole pause surface is the stock `PausableManager`, gated on DOM_PAUSER by the account's procedure-role map rather than on the owner | `XReserveAdminAuthority` (`admin_authority.rs`) + the composition in `XReserveStablecoinBuilder::build_components` (`builder/mod.rs`) | `builder_installs_the_stock_managers_with_their_roles_assigned` (`tests/builder_api.rs`) and 2 others |
...
| R-ADMIN-1 | `set_attester` is administrator-gated (it carries no role of its own, so the account's authority resolves it to the built-in `ADMIN` role, whose sole seeded member is the owner's account) | stock `ERR_SENDER_LACKS_ROLE` | `set_attester` (`attester_admin.masm`) | `set_attester_admin_note_owner_writes_and_nonowner_traps` (`tests/f5_admin_notes.rs`) and 2 others |
...
| CMP-F2 | The administrator-gated `set_min_burn_size` setter (note-driven, floor-guarded, calls the stock setter) | `main` (`asm/standards/notes/xreserve_set_min_burn_size_note.masm`) | `set_min_burn_owner_succeeds` (`tests/set_min_burn.rs`) and 7 others |
