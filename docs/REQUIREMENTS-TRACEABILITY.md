...
| INV-UINT256-TO-ASSETAMOUNT | uint256 reduction: assert high half zero, floor semantics at 10^scale, cap at `AssetAmount::MAX`; trap, never saturate. Rust floor-divides; MASM proves the quotient as a witness (standards multiply-and-verify) | `verify_uint256_to_asset_amount` (`encoding/mod.masm`) + Rust `uint256_to_asset_amount` (`amount.rs`) | `tv_dual_2_uint256_reducer` (`tests/masm_dual.rs`) and the `tv_amt_1..7` unit tests |
...
| DEV-5 | OPEN: cap `amount` at `AssetAmount::MAX` at 6-dp scale (provisional scale-0 identity); exact cap/scale/dust tolerance await Circle | `verify_uint256_to_asset_amount` (`encoding/mod.masm`) + Rust `uint256_to_asset_amount` (`amount.rs`); `DEPOSIT_SCALE_EXP` (`mint_policy.masm`) | `tv_dual_2_uint256_reducer` (`tests/masm_dual.rs`) and 8 others incl. the `mint_scale_conformance.rs` identity suite |
...
| R-MINT-9 | Reduced `amount`/`maxFee`/`feeAmount` fit ≤ 2^128 (high four limbs zero) | `ERR_X_TOO_LARGE` (maxFee/fee, parser-declared) and the standards verifier's own x bound (amount) | `assert_mint_amounts` (`deposit_intent_parser.masm`): the amount via `encoding::verify_uint256_to_asset_amount`, maxFee/fee via the local `to_u128_limbs` staging | `d5b_amount_fee_rejects::case::r_mint_9_amount_overflow` (+ maxfee/fee twins) and `tv_dual_2_uint256_reducer` |
...
| DC-5 | uint256 → AssetAmount: byte-swap, assert high 4 limbs zero, floor semantics at 10^scale, reject above `AssetAmount::MAX` | Rust `uint256_to_asset_amount` (`amount.rs`, floor-divide) / MASM `verify_uint256_to_asset_amount` (`encoding/mod.masm`, witness verify) | `tv_dual_2_uint256_reducer` and 5 others |
...
| TV-DUAL-2 | The Rust-computed quotient passes the MASM witness verifier / both sides trap alike on every conversion vector | Rust `uint256_to_asset_amount` + MASM `verify_uint256_to_asset_amount` | `tv_dual_2_uint256_reducer` |
...
| Q-CRY-6 | OPEN: the exact amount cap/scale — provisional `AssetAmount::MAX` cap at 6-dp denomination | `verify_uint256_to_asset_amount` (`encoding/mod.masm`; the cap is the standards verifier's y bound) | `mint_rejects_an_over_cap_amount` and 2 others |
