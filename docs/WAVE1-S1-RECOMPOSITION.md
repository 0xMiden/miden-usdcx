...
- **Attested-recipient extraction** (`mint_recipient_account_id.rs`) → pad guard:
  ...
  (originally `ERR_XRESERVE_RECIPIENT_NONCANONICAL` at both `build_felt` call sites; since the
  standards de-duplication the same rejects trap the linked `eth::build_felt`'s merge-overflow
  error). NOT re-driven e2e: the
