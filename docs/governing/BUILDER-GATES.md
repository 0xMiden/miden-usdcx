...
- **Naming decisions are binding.** **NS-1** (amended twice) → canonical MASM proc `xreserve::mint_intent::hashed_nonce` (do not add a MASM alias); **NS-2** is retired and **NS-3** replaces it → the on-chain DepositIntent realization is **01-owned `xreserve::deposit_intent::rebuild`**, a writer rather than a parser, while 04 still owns the layouts and the shared codecs. **DC-7** ships as the Rust burn-note codec at `encoding/burn_note.rs`; there is no `burn_items.masm`. Any future unadjudicated split in the ownership map is a STOP condition: the builder must return to the human with no silent pick, dual name, or local alias.
