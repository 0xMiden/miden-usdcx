...
- **`advice-provider-hygiene` is MANDATORY and security-critical for the `verify_attestation` proc and the admin key writes** (consortium H1). Every advice/`NoteAttachment`-sourced value — the 17-felt ECDSA signature, the 1-felt attester index, and the 17 note-storage items `set_attester` acts on — MUST be validated against a kernel-trusted commitment (the attachment and note-storage commitment checks the kernel performs on the way into memory, plus the keccak-bound signature), use content-addressed advice-map keys, and **ERROR (not default) on missing advice**. A missed advice-commitment / missing-advice check on this path is an unlimited-mint authorization bypass — the worst failure mode in the faucet. Under `DC-15` the attester **key itself is no longer advice-sourced at all**: it is account state, so the only thing the note can choose is which index to name.
