...
| **DepositIntent reconstruction from the carried mint intent; the mint-intent wire shape (DC-14)** | **faucet (01)** owns the writer and the carried shape (both are mint-transport concerns and the writer reads faucet state); **shared-encoding (04)** owns the felt offsets it writes at | `asm/standards/xreserve/deposit_intent.masm` → `xreserve::deposit_intent::rebuild`; the carried offsets in `mint_intent.masm` | `crates/xusdc-encoding/src/xreserve/encoding/mint_intent.rs` → `MintIntent::{from_deposit_intent, to_deposit_intent, to_felts, from_felts}` (the bytes come from `DepositIntent`'s `Serializable`, which owns them) | `DC-14`; `TV-DUAL-6` (the round-trip and MASM/Rust preimage-parity rows) |
