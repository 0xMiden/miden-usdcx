...
| `XReserveBurnNote` item codec `(amount,destDomain,destRecipient,salt)` (DC-7) | **shared-encoding (04)** owns the codec; **faucet (01)** owns note production/consumption policy | no MASM codec; carried in note attachment scheme 6 — 18 felts (`amount[0]`, `destDomain[1]`, `destRecipient[2..10]`, `salt[10..18]`) zero-padded to 5 words (2 pad felts), attachment slots `[scheme-2 routing, scheme-6 payload]`, decoder rejects any word count ≠ 5; the faucet uses the stock burn consume script | `crates/xusdc-encoding/src/xreserve/encoding/burn_note.rs`; listener decode | `DC-7`; `INV-PUBLIC-BURN-OBSERVABILITY` |
