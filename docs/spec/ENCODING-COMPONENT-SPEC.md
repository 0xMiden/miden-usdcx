...
| `uint256_to_asset_amount` (Rust) | The `DC-5` reduction (`INV-UINT256-TO-ASSETAMOUNT`), `y = floor(x / 10^scale_exp)` with `scale_exp` bounded to `0..=18`. The arithmetic IS the standards' `EthAmount::scale_to_asset_amount`, called directly; this routine only adapts it to `EncodingError`. **Rust-only:** the MASM witness verifier is removed, because under `DC-14` the uint256 never reaches the chain — see the ownership map's `DC-5` rider before reopening `DEV-5`. |
