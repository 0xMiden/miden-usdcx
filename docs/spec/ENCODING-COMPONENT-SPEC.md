...
| `DepositIntent` `Deserializable` / `TryFrom<&[u8]>` (Rust) | Structural DepositIntent validation (magic, version, non-zero `amount`/`localToken`/`localDepositor`, the length relation, the hookData bound). It does NOT narrow the four bytes32 fields — Circle's own encoder emits opaque bytes32 there (`DEV-10`, `Q-EVM-ADDR-1`), so that reading belongs to `MintIntent::from_deposit_intent`. **Rust-only since `DC-14`** — the on-chain parser is retired (`NS-2`), because the faucet writes those fields instead of reading them. It is the compress-side entry and the relayer's pre-validate (`INV-DEPOSITINTENT-PARSE`). |
