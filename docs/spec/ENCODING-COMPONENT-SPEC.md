...
| DC-7 | **Burn-note payload** — `(amount, destDomain, destRecipient, salt)` = 18 felts (`amount[0]`, `destDomain[1]`, `destRecipient[2..10]`, `salt[10..18]`), carried in note **attachment scheme 6**, zero-padded to **5 words** (`ceil(18/4)`, 2 pad felts). Burn-note attachment slots are `[scheme-2 routing, scheme-6 payload]`; the decoder rejects any word count ≠ 5. Shipped Rust-only; there is no `burn_items.masm`. |
