...
| Id | Condition enforced |
|---|---|
| R-ADMIN-1 | `set_attester` is administrator-gated: it carries no role of its own, so the account's role-based authority resolves it to the built-in `ADMIN` role — whose sole seeded member is the owner's account — and a sender without that role is rejected. |
...
|---|---|
| IMPL-DEV-1 | The sole pause surface is the stock `PausableManager`, gated on `DOM_PAUSER` by the account's per-procedure role map — so the owner, who administers everything else, has no pause path. |
...
| IMPL-DEV-3 | Per-setter admin roles were replaced with a single administrator gate: the setters carry no role of their own and resolve to the built-in `ADMIN` role, seeded on the owner's account (which also matches Circle). Note that `ADMIN` membership is account-bound and does NOT follow an ownership transfer — see `IMPL-DEV-23`. |
...
| IMPL-DEV-16 | `identifier_init` (formerly `domain_init`) uses `ownable2step::assert_sender_is_owner` directly while the setters use `authority::assert_authorized`. The two NO LONGER resolve to the same identity: `identifier_init` follows the owner slot, the setters resolve to the built-in `ADMIN` role, and `ADMIN` membership is account-bound so it does not follow an ownership transfer. |
